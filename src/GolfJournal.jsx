...
const DEFAULT_BAG = ["Driver", "3W", "5W", "4H", "5i", "6i", "7i", "8i", "9i", "PW", "52°", "56°", "60°", "Putter"];
...
const DUFF_ZONE = { key: "duffed", label: "Duffed it", color: COLORS.trouble };

const ALL_ZONES = [...FAIRWAY_ZONES, ...FAIRWAY_BUNKER_ZONES, DUFF_ZONE, GREEN_ZONE, ...BUNKER_ZONES, ...MISS_ZONES];
...
function ShotZoneSelector({ value, onChange }) {
  ...
  const yBottom = 520;
  const fairwayTop = CY + R_COLLAR_OUTER; // fairway meets the rough collar here
  const bottomHalf = 135;
  const topHalf = R_COLLAR_OUTER; // matches the collar diameter for a smooth join
  ...
  const fairwayGeom = FAIRWAY_ZONES.map((z, i) => {
    ...
    return { ...z, shape: "poly", points, cx, cy };
  });

  const midY = (yBottom + fairwayTop) / 2;
  const fbSeeds = [0.12, -0.1, 0.15, -0.08, 0.1];
  const fairwayBunkerGeom = [
    { ...FAIRWAY_BUNKER_ZONES[0], cx: CX - 65, cy: midY + 60 },
    { ...FAIRWAY_BUNKER_ZONES[1], cx: CX, cy: midY - 55 },
    { ...FAIRWAY_BUNKER_ZONES[2], cx: CX + 65, cy: midY + 60 },
  ].map((z, i) => ({ ...z, shape: "path", d: blobPath(z.cx, z.cy, 18, fbSeeds, i * 30) }));
  ...
  const allGeom = [...fairwayGeom, ...fairwayBunkerGeom, duffGeom, greenGeom, ...bunkerGeom, ...missGeom];
  ...
  return (
    <div className="tee-shot-wrap">
      <svg viewBox="0 0 300 600" className="tee-shot-svg" role="group" aria-label="Shot landing zone">
        {fairwayGeom.map((z) =>
          z.shape === "path" ? (
...
function HoleScreen({ round, holeIndex, onHoleChange, onNav, onGoReview, onEndRound, bag, autosaveStatus }) {
  ...
  const addShot = () => {
    ...
    const preselect = lastShot && !lastShot.isPenalty && lastShot.zone === "green"
      ? { club: "Putter", putts: 1 }
      : {};
    applyShots([...shots, { ...emptyShot(), ...preselect }]);
  };
  ...
  return (
    <div className="hole-screen">
      ...
      <div className="hole-nav">
        <button className="nav-btn" disabled={holeIndex === 0} onClick={() => { window.scrollTo({ top: 0, behavior: "instant" }); onNav(holeIndex - 1); }}>
          <ChevronLeft size={16} /> Hole {h.hole - 1 || ""}
        </button>
        <button className="nav-btn nav-btn-primary" onClick={() => { window.scrollTo({ top: 0, behavior: "instant" }); holeIndex === 17 ? onGoReview() : onNav(holeIndex + 1); }}>
  ...
  return (
    <div className="editor">
      ...
      <div className="section">
        ...
        {bag.length > 0 && (
          <button className="clear-bag-btn" onClick={() => onChange([])}>
            Clear all
          </button>
        )}
      </div>
...
export default function GolfJournal() {
  ...
  // Set to true when the user discards a round — prevents any queued autosave
  // from writing the round to the database after the discard decision.
  const discardingRef = React.useRef(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (err) throw err;
        const raw = data?.clubs && data.clubs.length ? data.clubs : DEFAULT_BAG;
        // Strip any club names that no longer exist in CLUB_OPTIONS (e.g. old GW/SW/LW)
        const saved = raw.filter((c) => CLUB_OPTIONS.includes(c));
        const cleaned = saved.length ? saved : DEFAULT_BAG;
        setBag(cleaned);
        setEditingBag(cleaned);
      } catch (e) {
  ...
  // discardingRef blocks any queued save from firing after a discard decision.
  useEffect(() => {
  ...
  const handleDiscardRound = async () => {
    // Block any pending autosave timer from firing after we clear editing.
    discardingRef.current = true;
    const roundId = editing?.id;
    setEditing(null);
    setShowEndPrompt(false);
    setView("list");
    // If the round was already autosaved to the DB, delete it now.
    if (roundId && (rounds || []).some((r) => r.id === roundId)) {
      try {
        await supabase.from("rounds").delete().eq("id", roundId);
        setRounds((prev) => (prev || []).filter((r) => r.id !== roundId));
      } catch (e) {
        console.error("discard delete failed:", e);
      }
    }
    // Reset flag so normal autosave works again for the next round.
    ...
    <div className="app">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .clear-bag-btn { margin-top: 12px; width: 100%; background: transparent; border: 1.5px solid ${COLORS.flag}66; color: ${COLORS.flag}; border-radius: 6px; padding: 9px; font-weight: 600; font-size: 13px; cursor: pointer; }
        .clear-bag-btn:hover { background: ${COLORS.flag}15; border-color: ${COLORS.flag}; }
      ...
      {view === "hole" && editing && (
        <HoleScreen
          ...
          onEndRound={() => { setConfirmDiscard(false); setShowEndPrompt(true); }}
          bag={bag}
      ...
      {showEndPrompt && (
        <div className="modal-backdrop" onClick={() => { setShowEndPrompt(false); setConfirmDiscard(false); }}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            {confirmDiscard ? (
              <>
                <div className="modal-title">Discard this round?</div>
                <p className="modal-body">This cannot be undone. Everything logged for this round will be permanently deleted.</p>
                <button className="modal-btn modal-btn-danger" onClick={handleDiscardRound}>Yes, discard it</button>
                <button className="modal-btn modal-btn-ghost" onClick={() => setConfirmDiscard(false)}>Go back</button>
              </>
            ) : (
              <>
                ...
                <button className="modal-btn modal-btn-danger" onClick={() => setConfirmDiscard(true)}>Discard round</button>
