...
const TROUBLE_FRONT_ZONE = { key: "trouble-front", label: "Front trouble", color: COLORS.trouble };
...
// The green surround is split into 8 equal directions (45° each). "front" is
// the side facing the tee, "back" is farthest away. All 8 are always valid
// "missed the green" outcomes; 6 of them (everything except dead straight
// front/back) also have a bunker sitting inside that same wedge, matching
// how bunkers are actually placed on real greens.
const MISS_DIRS = ["back", "back-right", "right", "front-right", "front", "front-left", "left", "back-left"];
const BUNKER_DIRS = ["back-right", "right", "front-right", "front-left", "left", "back-left"];

const MISS_LABELS = { front: "short", back: "long" };
const MISS_ZONES = MISS_DIRS.map((d) => ({
  key: `miss-${d}`,
  label: `Missed green — ${MISS_LABELS[d] || d.replace("-", " ")}`,
  color: COLORS.rough,
}));

const BUNKER_ZONES = BUNKER_DIRS.map((d) => ({
  key: `bunker-${d}`,
  label: `Greenside bunker (${d.replace("-", " ")})`,
  color: COLORS.sand,
}));

// A duffed shot is a bad mishit, not a hazard — kept visually distinct from
// the trouble zones (deep trees/water/OB) with a dull, scuffed-turf color.
const DUFF_ZONE = { key: "duffed", label: "Duffed it", color: COLORS.duffTurf };


const ALL_ZONES = [...FAIRWAY_ZONES, TROUBLE_FRONT_ZONE, ...FAIRWAY_BUNKER_ZONES, DUFF_ZONE, GREEN_ZONE, ...BUNKER_ZONES, ...MISS_ZONES];
...
function computeRoundStats(holes) {
  ...
  const frontStats = totalsFor(front);
  const backStats = totalsFor(back);

  // Per-par-type average score (relative to par)
  const byPar = {};
  holes.forEach((h) => {
    if (!byPar[h.par]) byPar[h.par] = { strokes: 0, par: 0, count: 0 };
    byPar[h.par].strokes += h.strokes || 0;
    byPar[h.par].par += h.par || 0;
    byPar[h.par].count += 1;
  });
  const avgByPar = Object.keys(byPar)
    .sort((a, b) => a - b)
    .map((par) => {
      const g = byPar[par];
      return { par: Number(par), avgToPar: (g.strokes - g.par) / g.count, count: g.count };
    });

  let fairwayHits = 0, fairwayEligible = 0;
  let missLeft = 0, missRight = 0;
  let approachShotsTotal = 0, approachHolesCount = 0;
  let girHit = 0, girEligible = 0;
  let leftBunkers = 0, rightBunkers = 0, allGreensideBunkers = 0, greensHit = 0, greensideMisses = 0;
  ...
  holes.forEach((h) => {
    ...
    const nonPenaltyNonPutter = shots.filter((s) => !s.isPenalty && s.club !== "Putter");
    const approachCount = nonPenaltyNonPutter.length;

    if (h.par !== 3) {
      fairwayEligible += 1;
      if (teeShot && !teeShot.isPenalty && (teeShot.zone === "fairway" || teeShot.zone === "green")) {
        fairwayHits += 1;
      } else if (teeShot && !teeShot.isPenalty && teeShot.zone && teeShot.zone.includes("left")) {
        missLeft += 1;
      } else if (teeShot && !teeShot.isPenalty && teeShot.zone && teeShot.zone.includes("right")) {
        missRight += 1;
      }
    }

    if (approachCount > 0) {
      approachShotsTotal += approachCount;
      approachHolesCount += 1;
      girEligible += 1;
      if (approachCount <= h.par - 2) girHit += 1;
    }

    shots.forEach((s) => {
      if (s.zone === "duffed") duffedCount += 1;
      if (s.zone === "green") greensHit += 1;
      if (s.zone && s.zone.startsWith("miss-")) greensideMisses += 1;
      if (s.zone === "bunker-front-left" || s.zone === "bunker-left" || s.zone === "bunker-back-left") {
        leftBunkers += 1;
        allGreensideBunkers += 1;
      } else if (s.zone === "bunker-front-right" || s.zone === "bunker-right" || s.zone === "bunker-back-right") {
        rightBunkers += 1;
        allGreensideBunkers += 1;
      }
    });
  ...
  const bunkerBase = allGreensideBunkers + greensHit + greensideMisses;
  const leftBunkerPct = pct(leftBunkers, bunkerBase - rightBunkers);
  const rightBunkerPct = pct(rightBunkers, bunkerBase - leftBunkers);

  return {
    frontToPar: frontStats.toPar,
    backToPar: backStats.toPar,
    avgByPar,
    fairwayHitPct: pct(fairwayHits, fairwayEligible),
    missLeftPct: pct(missLeft, fairwayEligible),
    missRightPct: pct(missRight, fairwayEligible),
    avgApproachShots: approachHolesCount ? approachShotsTotal / approachHolesCount : null,
    girPct: pct(girHit, girEligible),
    leftBunkerPct,
    rightBunkerPct,
    avgPutts: holes.length ? holes.reduce((s, h) => s + (h.putts || 0), 0) / holes.length : null,
    onePutts,
    threePutts,
    penaltyStrokes: holes.reduce((s, h) => s + (h.penalties || 0), 0),
    duffedCount,
  };
}
...
// Small organic "blob" outline through n points around (cx,cy), each
// point's radius perturbed by a seed multiplier — used for bunkers and the
// green so they read as hand-drawn shapes rather than perfect geometry.
function blobPath(cx, cy, baseR, seeds, rotationDeg = 0) {
...
function ShotZoneSelector({ value, onChange }) {
  ...
  const CY = 190;
  const R_GREEN = 34;
  const R_COLLAR_INNER = 46;
  const R_COLLAR_OUTER = 115;
  const COLLAR_FAIRWAY_GAP = 36; // visible daylight between the green complex and the fairway strip
  const yBottom = 520 + COLLAR_FAIRWAY_GAP;
  const fairwayTop = CY + R_COLLAR_OUTER + COLLAR_FAIRWAY_GAP;
  const bottomHalf = 135;
  const topHalf = R_COLLAR_OUTER;
  const frontTroubleDepth = 62; // how deep the water-hazard band cuts into the middle lanes
  const frontTroubleBottom = fairwayTop + frontTroubleDepth; // the band sits right where the fairway meets the green complex
  ...
  // Clean circular sector — straight radial dividers, perfect arcs, no
  // organic wobble, matching a simple flat "pie slice" look.
  ...
  // 8 equal 45° segments. 0 = back (away from tee), 90 = right,
  // 180 = front (toward tee), 270 = left.
  const SEGMENTS = [
    { dir: "back", start: -22.5, end: 22.5 },
    { dir: "back-right", start: 22.5, end: 67.5 },
    { dir: "right", start: 67.5, end: 112.5 },
    { dir: "front-right", start: 112.5, end: 157.5 },
    { dir: "front", start: 157.5, end: 202.5 },
    { dir: "front-left", start: 202.5, end: 247.5 },
    { dir: "left", start: 247.5, end: 292.5 },
    { dir: "back-left", start: 292.5, end: 337.5 },
  ];

  const missGeom = SEGMENTS.map((seg) => {
    const meta = MISS_ZONES.find((z) => z.key === `miss-${seg.dir}`);
    const mid = (seg.start + seg.end) / 2;
    const c = toPoint((R_COLLAR_INNER + R_COLLAR_OUTER) / 2, mid);
    return { ...meta, shape: "path", d: sectorPath(R_COLLAR_INNER, R_COLLAR_OUTER, seg.start, seg.end), cx: c.x, cy: c.y };
  });

  const bunkerSeeds = [0.14, -0.1, 0.16, -0.08, 0.12, -0.14];
  const bunkerR = (R_COLLAR_INNER + R_COLLAR_OUTER) / 2;
  const bunkerGeom = SEGMENTS.filter((seg) => BUNKER_DIRS.includes(seg.dir)).map((seg) => {
    const meta = BUNKER_ZONES.find((z) => z.key === `bunker-${seg.dir}`);
    const mid = (seg.start + seg.end) / 2;
    const c = toPoint(bunkerR, mid);
    return { ...meta, shape: "path", d: blobPath(c.x, c.y, 24, bunkerSeeds, mid), cx: c.x, cy: c.y };
  });

  const greenGeom = { ...GREEN_ZONE, shape: "circle", cx: CX, cy: CY, r: R_GREEN };

  // Width of the whole fairway strip at any y between fairwayTop and
  // yBottom, and the x position of a given fractional slice at that y —
  // used to build the front-trouble band below the middle lanes.
  const halfWidthAt = (y) => bottomHalf + (topHalf - bottomHalf) * ((yBottom - y) / (yBottom - fairwayTop));
  const laneXAt = (y, frac) => CX - halfWidthAt(y) + frac * 2 * halfWidthAt(y);
  ...
  const fairwayGeom = FAIRWAY_ZONES.map((z, i) => {
    ...
    const cy = (yBottom + fairwayTop) / 2;
    // Give the two outer "trouble" lanes a wavy outer edge for character;
    // the inner boundary they share with the rough lanes stays straight.
    // These two run the full height, uninterrupted by the front-trouble
    // band below — they continue straight down past it to the tee.
    if (z.key === "trouble-left" || z.key === "trouble-right") {
      const isLeft = z.key === "trouble-left";
      const outerX = isLeft ? bl : br;
      const outerTopX = isLeft ? tl : tr;
      const innerX = isLeft ? br : bl;
      const innerTopX = isLeft ? tr : tl;
      const waveAmp = 11;
      const steps = 10;
      // Envelope tapers the wave to exactly 0 at both ends, so the wavy
      // edge joins the straight tee/green boundaries with no kink.
      const wavePts = [];
      for (let s = 0; s <= steps; s++) {
        const t = s / steps;
        const y = yBottom + (fairwayTop - yBottom) * t;
        const baseX = outerX + (outerTopX - outerX) * t;
        const envelope = Math.sin(t * Math.PI);
        const wave = Math.sin(t * Math.PI * 3 + (isLeft ? 0 : 2)) * waveAmp * envelope;
        wavePts.push([baseX + (isLeft ? -Math.abs(wave) : Math.abs(wave)), y]);
      }
      let d = `M ${innerX} ${yBottom} L ${wavePts[0][0]} ${wavePts[0][1]}`;
      for (let i = 0; i < wavePts.length - 1; i++) {
        const cur = wavePts[i], next = wavePts[i + 1];
        if (i === wavePts.length - 2) {
          d += ` L ${next[0]} ${next[1]}`;
        } else {
          const mid = [(cur[0] + next[0]) / 2, (cur[1] + next[1]) / 2];
          d += ` Q ${cur[0]} ${cur[1]} ${mid[0]} ${mid[1]}`;
      ...
      d += ` L ${innerTopX} ${fairwayTop} Z`;
      return { ...z, shape: "path", d, cx, cy };
    }

    // The 3 middle lanes (rough-left, fairway, rough-right) now start
    // below the water hazard rather than right at the green — the hazard
    // crosses in front of the green, and these lanes pick up after it.
    const bl2 = laneXAt(frontTroubleBottom, i / 5);
    const br2 = laneXAt(frontTroubleBottom, (i + 1) / 5);
    const points = `${bl},${yBottom} ${br},${yBottom} ${br2},${frontTroubleBottom} ${bl2},${frontTroubleBottom}`;
    return { ...z, shape: "poly", points, cx: (bl + br + bl2 + br2) / 4, cy: (yBottom + frontTroubleBottom) / 2 };
  });

  // Water hazard / ditch, spanning just the middle 3 lanes right where the
  // fairway meets the green — the two outer trouble lanes run past it
  // uninterrupted on either side, per the sketch.
  const frontTroubleGeom = (() => {
    const tl2 = laneXAt(fairwayTop, 1 / 5);
    const tr2 = laneXAt(fairwayTop, 4 / 5);
    const bl2 = laneXAt(frontTroubleBottom, 1 / 5);
    const br2 = laneXAt(frontTroubleBottom, 4 / 5);
    const points = `${bl2},${frontTroubleBottom} ${br2},${frontTroubleBottom} ${tr2},${fairwayTop} ${tl2},${fairwayTop}`;
    return { ...TROUBLE_FRONT_ZONE, shape: "poly", points, cx: CX, cy: (frontTroubleBottom + fairwayTop) / 2 };
  })();

  // Bunkers sit inside the rough-left/fairway/rough-right lanes, which now
  // start below the water hazard rather than at yBottom — position them
  // relative to that region so they don't end up inside the hazard.
  const midLaneY = (frontTroubleBottom + yBottom) / 2;
  const fbSeeds = [0.12, -0.1, 0.15, -0.08, 0.1];
  const fairwayBunkerGeom = [
    { ...FAIRWAY_BUNKER_ZONES[0], cx: CX - 65, cy: midLaneY + 42 },
    { ...FAIRWAY_BUNKER_ZONES[1], cx: CX, cy: midLaneY - 40 },
    { ...FAIRWAY_BUNKER_ZONES[2], cx: CX + 65, cy: midLaneY + 42 },
  ].map((z, i) => ({ ...z, shape: "path", d: blobPath(z.cx, z.cy, 16, fbSeeds, i * 30) }));

  // "Duffed It" zone — flush against the fairway's bottom edge. The top is
  // a perfectly straight line (no seam) because the curve is a wobbly
  // half-ellipse that naturally returns to exactly y=yBottom at both ends;
  // only the sides/bottom bulge outward organically.
  const duffHalfWidth = bottomHalf;
  const duffDepth = 46;
  const duffSteps = 24;
  const duffPts = [];
  for (let i = 0; i <= duffSteps; i++) {
    const t = i / duffSteps;
    const envelope = Math.sin(t * Math.PI);
    const dx = CX - duffHalfWidth * Math.cos(t * Math.PI) + envelope * 7 * Math.sin(t * Math.PI * 2.6);
    const dy = yBottom + envelope * (duffDepth + 9 * Math.sin(t * Math.PI * 3.2 + 1.1));
    duffPts.push([dx, dy]);
  }
  let duffD = `M ${duffPts[0][0]} ${duffPts[0][1]}`;
  for (let i = 0; i < duffPts.length - 1; i++) {
    const cur = duffPts[i], next = duffPts[i + 1];
    if (i === duffPts.length - 2) {
      duffD += ` L ${next[0]} ${next[1]}`;
    } else {
      const mid = [(cur[0] + next[0]) / 2, (cur[1] + next[1]) / 2];
      duffD += ` Q ${cur[0]} ${cur[1]} ${mid[0]} ${mid[1]}`;
    }
  }
  duffD += " Z";
  const duffGeom = { ...DUFF_ZONE, shape: "path", d: duffD, cx: CX, cy: yBottom + duffDepth * 0.6 };

  const allGeom = [...fairwayGeom, frontTroubleGeom, ...fairwayBunkerGeom, duffGeom, greenGeom, ...bunkerGeom, ...missGeom];
  const selected = allGeom.find((z) => z.key === value);

  const shapeProps = (z, opts = {}) => ({
    fill: z.color,
    opacity: value && value !== z.key ? 0.55 : 1,
    stroke: value === z.key ? COLORS.sand : (opts.baseStroke ?? COLORS.fairwayDeep),
    strokeWidth: value === z.key ? 4 : (opts.baseStroke === "none" ? 0 : 1.5),
  ...
  return (
    <div className="tee-shot-wrap">
      <svg viewBox="0 0 300 660" className="tee-shot-svg" role="group" aria-label="Shot landing zone">
        {fairwayGeom.map((z) =>
          z.shape === "path" ? (
        ...
        <polygon points={frontTroubleGeom.points} {...shapeProps(frontTroubleGeom)} />
        {fairwayBunkerGeom.map((z) => (
          <path key={z.key} d={z.d} {...shapeProps(z)} />
        ))}
        <path d={duffGeom.d} {...shapeProps(duffGeom)} />
        <text
          x={duffGeom.cx}
          y={duffGeom.cy + 5}
          textAnchor="middle"
          ...
          fontWeight={700}
          fontSize={20}
          fill={COLORS.scorecard}
          pointerEvents="none"
        >
          Duffed It
        </text>
        {/* Rough collar segments — visible thin dividers between each,
            like a pie sliced into wedges. */}
        {missGeom.map((z) => (
          <path key={z.key} d={z.d} {...shapeProps(z, { baseStroke: COLORS.fairwayDeep })} />
        ))}
        {bunkerGeom.map((z) => (
          <path key={z.key} d={z.d} {...shapeProps(z, { baseStroke: COLORS.sandDark })} />
        ))}
        <circle cx={greenGeom.cx} cy={greenGeom.cy} r={greenGeom.r} {...shapeProps(greenGeom)} />
        {/* flag + soft shadow, decorative only */}
        <ellipse cx={CX + 3} cy={CY + 3} rx={5} ry={2} fill={COLORS.fairwayDeep} opacity={0.3} pointerEvents="none" />
        <line x1={CX} y1={CY} x2={CX} y2={CY - 48} stroke={COLORS.scorecard} strokeWidth={2} pointerEvents="none" />
        <path d={`M ${CX} ${CY - 48} L ${CX} ${CY - 35} L ${CX + 14} ${CY - 42} Z`} fill={COLORS.flag} pointerEvents="none" />
...
function HoleScreen({ round, holeIndex, onHoleChange, onNav, onGoReview, onEndRound, bag, autosaveStatus }) {
  ...
  const addShot = () => {
    ...
    const onGreen = lastShot && lastShot.zone === "green";
    const nextShot = onGreen && bag.includes("Putter") ? { ...emptyShot(), club: "Putter" } : emptyShot();
    applyShots([...shots, nextShot]);
  };
  ...
  const autosaveLabel = autosaveStatus === "saving" ? "Saving..." : autosaveStatus === "saved" ? "Saved" : autosaveStatus === "error" ? "Not saved — check connection" : null;
  ...
  return (
    <div className="hole-screen">
      <div className="hole-topbar">
        ...
        {autosaveLabel && (
          <span className={`autosave-indicator ${autosaveStatus === "error" ? "autosave-error" : ""}`}>{autosaveLabel}</span>
        )}
        <div className="hole-topbar-actions">
          <button className="review-link" onClick={onGoReview}>
            <ClipboardList size={14} /> Scorecard
          </button>
          <button className="end-round-link" onClick={onEndRound}>
            End round
          </button>
        </div>
      </div>
            ...
            {shot.isPenalty ? (
              ...
              <>
                ...
                {shot.club === "Putter" ? (
                  ...
                  <>
                    <div className="section-label" style={{ marginTop: 6 }}>{label} — result</div>
...
function StatLine({ label, value }) {
  return (
    <div className="stat-line">
      ...
      <span className="stat-line-val">{value}</span>
    </div>
  );
}
...
function RoundStats({ holes }) {
  ...
  const fmtToPar = (v) => (v === null || Number.isNaN(v) ? "—" : v > 0 ? `+${v}` : v === 0 ? "E" : `${v}`);
  const fmtAvg = (v) => (v === null ? "—" : v.toFixed(1));

  return (
    <div className="section">
      <div className="section-label">Round stats</div>

      <div className="stat-group">
        <div className="stat-group-title">Scoring</div>
        <StatLine label="Front 9" value={fmtToPar(s.frontToPar)} />
        <StatLine label="Back 9" value={fmtToPar(s.backToPar)} />
        {s.avgByPar.map((g) => (
          <StatLine key={g.par} label={`Par ${g.par}s avg (${g.count})`} value={fmtToPar(Math.round(g.avgToPar * 10) / 10)} />
        ))}
      </div>

      <div className="stat-group">
        <div className="stat-group-title">Tee shots</div>
        <StatLine label="Fairways hit" value={fmtPct(s.fairwayHitPct)} />
        <StatLine label="Missed left" value={fmtPct(s.missLeftPct)} />
        <StatLine label="Missed right" value={fmtPct(s.missRightPct)} />
      </div>

      <div className="stat-group">
        <div className="stat-group-title">Approach</div>
        <StatLine label="Avg tee-to-green shots" value={fmtAvg(s.avgApproachShots)} />
        <StatLine label="Greens in regulation" value={fmtPct(s.girPct)} />
        <StatLine label="Greenside bunkers (left)" value={fmtPct(s.leftBunkerPct)} />
        <StatLine label="Greenside bunkers (right)" value={fmtPct(s.rightBunkerPct)} />
      </div>

      <div className="stat-group">
        <div className="stat-group-title">Putting</div>
        <StatLine label="Avg putts / hole" value={fmtAvg(s.avgPutts)} />
        <StatLine label="1-putts" value={s.onePutts} />
        <StatLine label="3-putts or more" value={s.threePutts} />
      </div>

      <div className="stat-group">
        <div className="stat-group-title">Mistakes</div>
        <StatLine label="Penalty strokes" value={s.penaltyStrokes} />
        <StatLine label="Duffed shots" value={s.duffedCount} />
      </div>
    </div>
  );
}

function ReviewScreen({ round, onEditHole, onNotesChange, onSave, onBackToHole, onEndRound }) {
...
export default function GolfJournal() {
  ...
  // Shared upsert used by both the explicit "Save round" action and the
  // silent autosave below, so a crash or a forgotten save never loses data —
  // whatever's on screen is already sitting in the database.
  const persistRound = async (round) => {
    const row = {
      id: round.id,
      user_id: userId,
    ...
    setRounds((prev) => {
      ...
      return [summary, ...rest].sort((a, b) => (a.date < b.date ? 1 : -1));
    });
    return saved;
  };
  ...
  // Autosave whatever's being played, a moment after each change — so if the
  // app crashes or the person just closes the tab, the round is still there
  // next time rather than depending on them remembering to hit Save.
  useEffect(() => {
    if (!editing || !userId || view === "setup") return;
    setAutosaveStatus("saving");
    const timeout = setTimeout(async () => {
      try {
        await persistRound(editing);
    ...
    <div className="app">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .header-row { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px; max-width: 640px; margin: 0 auto 20px; }
        ...
        .home-header { display: flex; flex-direction: column; align-items: center; text-align: center; gap: 6px; max-width: 640px; margin: 8px auto 24px; color: ${COLORS.sand}; }
        .home-title { font-family: 'Fraunces', serif; font-weight: 700; font-size: 30px; letter-spacing: -0.01em; color: ${COLORS.scorecard}; }
        .home-actions { display: flex; flex-direction: column; gap: 12px; max-width: 280px; margin: 0 auto 28px; }
        .home-action-btn { width: 100%; background: transparent; border: 1.5px solid ${COLORS.sand}66; color: ${COLORS.scorecard}; border-radius: 999px; padding: 13px 18px; font-weight: 600; font-size: 15px; display: flex; align-items: center; justify-content: center; gap: 8px; cursor: pointer; transition: border-color 0.15s ease, background 0.15s ease; }
        .home-action-btn:hover { border-color: ${COLORS.sand}; background: ${COLORS.sand}0f; }
        .home-action-primary { background: ${COLORS.turf}; border-color: ${COLORS.turf}; color: ${COLORS.scorecard}; }
        .home-action-primary:hover { background: ${COLORS.turfLight}; border-color: ${COLORS.turfLight}; }
        .home-action-signout { border-color: transparent; color: ${COLORS.scorecard}77; font-size: 13px; padding: 8px; }
        .home-action-signout:hover { color: ${COLORS.flag}; border-color: transparent; background: none; }
        ...
        .editor-top { margin-bottom: 14px; display: flex; justify-content: space-between; align-items: center; }
        ...
        .course-input { background: ${COLORS.scorecardDark}; border: none; border-radius: 4px; padding: 10px 12px; font-family: 'Fraunces', serif; font-weight: 600; color: ${COLORS.ink}; width: 100%; }
        .date-input { background: ${COLORS.scorecardDark}; border: none; border-radius: 4px; padding: 10px 12px; font-family: 'IBM Plex Mono', monospace; font-size: 13px; font-weight: 500; color: ${COLORS.ink}; width: auto; max-width: 160px; }

        .hole-screen { max-width: 480px; margin: 0 auto; }
        .hole-topbar { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; gap: 8px; flex-wrap: wrap; }
        .autosave-indicator { font-size: 11px; color: ${COLORS.turfLight}; font-weight: 500; }
        .autosave-error { color: ${COLORS.flag}; font-weight: 600; }
        .hole-topbar-actions { display: flex; align-items: center; gap: 14px; }
        .end-round-link { background: none; border: none; color: ${COLORS.flag}; font-size: 12px; font-weight: 600; cursor: pointer; padding: 4px; }
        .end-round-link:hover { text-decoration: underline; }

        .modal-backdrop { position: fixed; inset: 0; background: rgba(15,36,29,0.75); display: flex; align-items: center; justify-content: center; padding: 24px; z-index: 50; }
        .modal-card { background: ${COLORS.scorecard}; color: ${COLORS.ink}; border-radius: 10px; padding: 22px 20px; width: 100%; max-width: 340px; box-shadow: 0 10px 30px rgba(0,0,0,0.4); }
        .modal-title { font-family: 'Fraunces', serif; font-weight: 700; font-size: 19px; margin-bottom: 8px; }
        .modal-body { font-size: 13px; color: ${COLORS.ink}99; margin: 0 0 18px; line-height: 1.4; }
        .modal-btn { width: 100%; border: none; border-radius: 6px; padding: 13px; font-weight: 600; font-size: 14px; cursor: pointer; margin-bottom: 10px; }
        .modal-btn-primary { background: ${COLORS.turf}; color: ${COLORS.scorecard}; }
        .modal-btn-primary:hover { background: ${COLORS.turfLight}; }
        .modal-btn-danger { background: transparent; border: 1.5px solid ${COLORS.flag}; color: ${COLORS.flag}; }
        .modal-btn-danger:hover { background: ${COLORS.flag}15; }
        .modal-btn-ghost { background: none; color: ${COLORS.ink}88; margin-bottom: 0; }
        .modal-btn-ghost:hover { color: ${COLORS.ink}; }
        ...
        .stat-group-title { font-family: 'Fraunces', serif; font-weight: 700; font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; color: ${COLORS.ink}; margin-bottom: 4px; }
        .stat-line { display: flex; justify-content: space-between; align-items: center; padding: 5px 0; border-bottom: 1px dashed ${COLORS.ink}18; font-size: 13px; }
        .stat-line:last-child { border-bottom: none; }
        .stat-line-label { color: ${COLORS.ink}99; }
        .stat-line-val { font-family: 'IBM Plex Mono', monospace; font-weight: 700; color: ${COLORS.ink}; }
        ...
        @media (max-width: 400px) {
          ...
          .new-btn, .courses-link-btn { padding: 8px 12px; font-size: 12px; }
      ...
      {view === "list" && (
        <>
          <div className="home-header">
            <Flag size={34} strokeWidth={2.2} />
            <div className="home-title">Golf Journal</div>
          </div>
          <div className="home-actions">
            <button className="home-action-btn home-action-primary" onClick={startNew}><Plus size={16} /> New round</button>
            <button className="home-action-btn" onClick={() => { setEditingBag(bag); setView("bag-editor"); }}>My Bag</button>
            <button className="home-action-btn" onClick={() => setView("courses-list")}><MapPin size={15} /> Courses</button>
            <button className="home-action-btn home-action-signout" onClick={() => supabase.auth.signOut()} title={session.user.email}>
      ...
      {view === "setup" && editing && (
        <RoundSetup round={editing} onChange={setEditing} onBegin={() => { setHoleIndex(0); setView("hole"); }} onCancel={() => { setEditing(null); setView("list"); }} courses={courses || []} />
      )}
      ...
      {view === "hole" && editing && (
        <HoleScreen
          ...
          onEndRound={() => setShowEndPrompt(true)}
          bag={bag}
      ...
      {showEndPrompt && (
        <div className="modal-backdrop" onClick={() => setShowEndPrompt(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="modal-title">End this round?</div>
            <p className="modal-body">You can save what you've logged so far, or discard it completely.</p>
            <button className="modal-btn modal-btn-primary" onClick={handleSaveAndEndRound}>Save round</button>
            <button className="modal-btn modal-btn-danger" onClick={handleDiscardRound}>Discard round</button>
            <button className="modal-btn modal-btn-ghost" onClick={() => setShowEndPrompt(false)}>Keep playing</button>
          </div>
        </div>
      )}
    </div>
