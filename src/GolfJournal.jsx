...
function ShotZoneSelector({ value, onChange }) {
  ...
  const frontTroubleDepth = 62; // how deep the new front-trouble band cuts into the middle lanes
  const frontTroubleTop = yBottom - frontTroubleDepth;
  ...
  const fairwayGeom = FAIRWAY_ZONES.map((z, i) => {
    ...
    // The 3 middle lanes (rough-left, fairway, rough-right) now stop short
    // of the tee, making room for the new front-trouble band beneath them.
    const bl2 = laneXAt(frontTroubleTop, i / 5);
    const br2 = laneXAt(frontTroubleTop, (i + 1) / 5);
    const points = `${bl2},${frontTroubleTop} ${br2},${frontTroubleTop} ${tr},${fairwayTop} ${tl},${fairwayTop}`;
    return { ...z, shape: "poly", points, cx: (bl2 + br2 + tl + tr) / 4, cy: (frontTroubleTop + fairwayTop) / 2 };
  });

  // New "front trouble" band, spanning just the middle 3 lanes near the
  // tee — the two outer trouble lanes flank it and continue past it
  // uninterrupted, per the sketch.
  const frontTroubleGeom = (() => {
    const tl2 = laneXAt(frontTroubleTop, 1 / 5);
    const tr2 = laneXAt(frontTroubleTop, 4 / 5);
    const bl2 = laneXAt(yBottom, 1 / 5);
    const br2 = laneXAt(yBottom, 4 / 5);
    const points = `${bl2},${yBottom} ${br2},${yBottom} ${tr2},${frontTroubleTop} ${tl2},${frontTroubleTop}`;
    return { ...TROUBLE_FRONT_ZONE, shape: "poly", points, cx: CX, cy: (yBottom + frontTroubleTop) / 2 };
  })();

  // Bunkers sit inside the rough-left/fairway/rough-right lanes, which now
  // stop at frontTroubleTop rather than yBottom — position them relative to
  // that shortened region so they don't end up inside the new trouble band.
  const midLaneY = (fairwayTop + frontTroubleTop) / 2;
