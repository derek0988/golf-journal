...
const emptyShot = () => ({ id: `shot_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, club: "", zone: null });
...
function migrateHole(h) {
  if (h.shots && h.shots.length > 0) return h;
  const shots = h.teeClub || h.teeShot ? [{ id: `shot_${h.hole}_migrated`, club: h.teeClub || "", zone: h.teeShot || null }] : [emptyShot()];
  return { ...h, shots };
}
...
function HoleScreen({ round, holeIndex, onHoleChange, onNav, onGoReview }) {
  ...
  const shots = h.shots || [];
  const updateShot = (idx, p) => patch({ shots: shots.map((s, i) => (i === idx ? { ...s, ...p } : s)) });
  const addShot = () => patch({ shots: [...shots, emptyShot()] });
  const removeShot = (idx) => {
    const next = shots.filter((_, i) => i !== idx);
    patch({ shots: next.length ? next : [emptyShot()] });
  };
      ...
      {shots.map((shot, idx) => (
        <div className="section" key={shot.id}>
          <div className="section-label-row">
            <span className="section-label">{idx === 0 ? "Shot 1 (tee) — club" : `Shot ${idx + 1} — club`}</span>
            {shots.length > 1 && (
              <button className="remove-shot-btn" onClick={() => removeShot(idx)} aria-label={`remove shot ${idx + 1}`}>
                <X size={13} />
              </button>
            )}
          </div>
          <div className="club-grid">
            {CLUBS.map((c) => (
              <button
                ...
                onClick={() => updateShot(idx, { club: shot.club === c ? "" : c })}
              >
                {c}
              </button>
            ))}
          ...
          <div className="section-label" style={{ marginTop: 14 }}>
            {idx === 0 ? "Shot 1 (tee) — result" : `Shot ${idx + 1} — result`}
          </div>
          ...
          <div className="tee-rating-row">
            <div className="tee-rating-field">
              ...
              <input
                ...
                step="0.1"
                value={tee.rating}
                onChange={(e) => updateTee({ rating: parseFloat(e.target.value) || 0 })}
              />
            </div>
            <div className="tee-rating-field">
              ...
              <input
                ...
                type="number"
                value={tee.slope}
                onChange={(e) => updateTee({ slope: parseInt(e.target.value, 10) || 0 })}
              />
            </div>
          </div>
        </div>
...
export default function GolfJournal() {
  ...
  const [view, setView] = useState("list"); // list | setup | hole | review | courses-list | course-editor
